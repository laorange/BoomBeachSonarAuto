...
- 当前已完全支持 1 至 ~~11~~ 50 号海域的自动化搜索算法探索。（其实就是复用14号海域，后续海域重复无变化）
- 添加对自动化搜索算法的单元测试。
---
...
如果探测异常中断并保留了 DROP，程序会弹出 Windows 是/否对话框，询问是否按你的人工判断恢复普通网络。选择“是”后只清除 REJECT/DROP 并复查 IPv4/IPv6 规则，不会识别、点击、启动或登录游戏；选择“否”则保持现场不变，不需要运行任何恢复命令。
